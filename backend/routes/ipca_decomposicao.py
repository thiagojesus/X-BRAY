...
@router.get("/grupos")
def get_ipca_grupos():
    data = fetch_sgs_batch(IPCA_GROUPS, start_date="01/01/2015")
    return {"source": "BCB SGS", "type": "grupos_despesa", "data": data}


@router.get("/naturezas")
def get_ipca_naturezas():
    data = fetch_sgs_batch(IPCA_NATURE, start_date="01/01/2015")
    return {"source": "BCB SGS", "type": "naturezas", "data": data}


@router.get("/core")
def get_ipca_core():
    data = fetch_sgs_batch(IPCA_CORE, start_date="01/01/2015")
    return {"source": "BCB SGS", "type": "core", "data": data}


@router.get("/precos")
def get_ipca_precos():
    data = fetch_sgs_batch(IPCA_PRICES, start_date="01/01/2015")
    return {"source": "BCB SGS", "type": "livres_administrados", "data": data}


@router.get("/tudo")
def get_ipca_tudo():
    grupos = fetch_sgs_batch(IPCA_GROUPS, start_date="01/01/2015")
    naturezas = fetch_sgs_batch(IPCA_NATURE, start_date="01/01/2015")
    core = fetch_sgs_batch(IPCA_CORE, start_date="01/01/2015")
    precos = fetch_sgs_batch(IPCA_PRICES, start_date="01/01/2015")
    return {
        "source": "BCB SGS",
        "grupos": grupos,
        "naturezas": naturezas,
